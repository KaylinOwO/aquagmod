namespace offsets
{
	namespace DT_BasePlayer
	{
		int m_iFOV = 0;
		int m_iFOVStart = 0;
		int m_flFOVTime = 0;
		int m_bIsScoped = 0;
		int m_iDefaultFOV = 0;
		int m_hZoomOwner = 0;
		int m_hVehicle = 0;
		int m_hUseEntity = 0;
		int m_iHealth = 0;
		int m_lifeState = 0;
		int m_iBonusProgress = 0;
		int m_iBonusChallenge = 0;
		int m_flMaxspeed = 0;
		int m_fFlags = 0;
		int m_iObserverMode = 0;
		int m_hObserverTarget = 0;
		int m_szLastPlaceName = 0;
		int m_bUseWeaponsInVehicle = 0;
	};
	namespace DT_HL2MP_Player
	{
		int m_hRagdoll = 0;
		int m_iSpawnInterpCounter = 0;
		int m_iPlayerSoundType = 0;
		int m_fIsWalking = 0;
	};
	namespace DT_GMOD_Player
	{
		int m_iGModPlayerFlags = 0;
		int m_HoveredWidget = 0;
		int m_PressedWidget = 0;
		int m_Driving = 0;
		int m_DrivingMode = 0;
		int m_PlayerClass = 0;
		int m_bCanZoom = 0;
		int m_bCanWalk = 0;
		int m_bIsTyping = 0;
		int m_StepSize = 0;
		int m_JumpPower = 0;
		int m_ViewOffset = 0;
		int m_ViewOffsetDucked = 0;
		int m_fGestureEndTime = 0;
		int m_PlayerColor = 0;
		int m_WeaponColor = 0;
		int m_Hands = 0;
		int m_nWaterLevel = 0;
	};
	namespace DT_BaseEntity
	{
		int m_flSimulationTime = 0;
		int m_ubInterpolationFrame = 0;
		int m_vecOrigin = 0;
		int m_angRotation = 0;
		int m_nModelIndex = 0;
		int m_fEffects = 0;
		int m_nRenderMode = 0;
		int m_nRenderFX = 0;
		int m_clrRender = 0;
		int m_iTeamNum = 0;
		int m_CollisionGroup = 0;
		int m_flElasticity = 0;
		int m_flShadowCastDistance = 0;
		int m_hOwnerEntity = 0;
		int m_hEffectEntity = 0;
		int moveparent = 0;
		int m_iParentAttachment = 0;
		int m_Collision = 0;
		int m_iTextureFrameIndex = 0;
		int m_bSimulatedEveryTick = 0;
		int m_bAnimatedEveryTick = 0;
		int m_bAlternateSorting = 0;
		int m_takedamage = 0;
		int m_RealClassName = 0;
		int m_OverrideMaterial = 0;
		int m_iHealth = 0;
		int m_iMaxHealth = 0;
		int m_spawnflags = 0;
		int m_iGModFlags = 0;
		int m_bOnFire = 0;
		int m_vecPunchAngleVel = 0;
		int m_vecPunchAngle = 0;
		int m_CreationTime = 0;
		int m_vecVelocity0 = 0;
		int m_vecVelocity1 = 0;
		int m_vecVelocity2 = 0;
		int m_GMOD_DataTable = 0;
		int m_GMOD_bool = 0;
		int m_GMOD_float = 0;
		int m_GMOD_int = 0;
		int m_GMOD_Vector = 0;
		int m_GMOD_QAngle = 0;
		int m_GMOD_EHANDLE = 0;
		int m_GMOD_String0 = 0;
		int m_GMOD_String1 = 0;
		int m_GMOD_String2 = 0;
		int m_GMOD_String3 = 0;
	};
	namespace DT_BaseCombatCharacter
	{
		int m_hActiveWeapon = 0;
		int m_hMyWeapons = 0;
	};
	namespace DT_BaseAnimating
	{
		int m_nSequence = 0;
		int m_nForceBone = 0;
		int m_vecForce = 0;
		int m_nSkin = 0;
		int m_nBody = 0;
		int m_nHitboxSet = 0;
		int m_flModelScale = 0;
		int m_flModelWidthScale = 0;
		int m_flPoseParameter = 0;
		int m_flPlaybackRate = 0;
		int m_flEncodedController = 0;
		int m_bClientSideAnimation = 0;
		int m_bClientSideFrameReset = 0;
		int m_nNewSequenceParity = 0;
		int m_nResetEventsParity = 0;
		int m_nMuzzleFlashParity = 0;
		int m_hLightingOrigin = 0;
		int m_hLightingOriginRelative = 0;
		int m_fadeMinDist = 0;
		int m_flFadeScale = 0;
		int m_pBoneManipulator = 0;
		int m_pFlexManipulator = 0;
		int m_OverrideViewTarget = 0;
	};
};